...
def get_logger(file_name, data_name):
    ...
    filename = "./in_results/" + data_name  + ".log"
...
def get_mask(view_num, data_len, missing_rate):
    ...
    error = 1
    if one_rate == 1:                                   
        matrix = np.ones((data_len, view_num)).astype(np.int64)         
        return matrix
    while error >= 0.005:
        enc = OneHotEncoder()
        view_preserve = enc.fit_transform(randint(0, view_num, size=(data_len, 1))).toarray()
...
class AnyDataset(Dataset):
    def __init__(self, dataname, missing_rate):
        ...
        self.mask = get_mask(self.v, len(self.gnd), missing_rate=missing_rate)
    ...
    def __getitem__(self, idx):
        ...
        for v in range(self.v):
            return_data.append(torch.from_numpy(np.array(self.features[0][v][idx],dtype=np.float32)))
        return return_data, torch.from_numpy(np.array(self.gnd[idx])), torch.from_numpy(np.array(idx)), torch.from_numpy(np.array(self.iden[idx])), torch.from_numpy(self.mask[idx])
def dataset_with_info(dataname, missing_rate=0.1):
    features, gnd = loadData(path + dataname + '.mat') 
    print(features.shape)
    views = max(features.shape[0],features.shape[1])
    input_num = features[0][0].shape[0]
    datasetforuse = AnyDataset(dataname, missing_rate=missing_rate)
...
class TESTNet(nn.Module):
    ...
    def forward(self, xs,clustering=False,):
        zs=[]
        zs_svd=[]
        zs_r=[]
...
dataname = "Scene15"#LandUse-21(2100),WikipediaArticles(693),BBCSport(544),synthetic3d(600),prokaryotic(551),handwritten(2000),100leaves(1600),handwritten、Mfeat(2000)
...
missing_rate = 0.7
datasetforuse, data_size, view_num, nc, input_dims, gnd = dataset_with_info(dataname, missing_rate=missing_rate)
...
def main():  
    ...
    low_feature_dim = 512#256  
                     
    h_dims = [1024, 1024]      #h_dims = [1024, 1024]        
    device="cuda:0"#device="cuda:0"
    train_loader = DataLoader(datasetforuse, batch_size=batch_size, shuffle=True, drop_last=True) #drop_last=False
    ...
    settings = {"seed": seed, "batch_size": batch_size, "dataname": dataname, "learning_rate": lr, "h_dims": h_dims, "low_feature_dim": low_feature_dim, 
                "epochs": train_epoch,"mcl_weight":mcl_weight,"mcl_cluster_weight":mcl_cluster_weight,"missing_rate":missing_rate,"device": device}
    ...
    for epoch in range(train_epoch):
        ...
        for xs, _, idx, inpu,mask in train_loader:
        
            
            model.train()
            for v in range(view_num):
                xs[v] = (xs[v] * mask[:, v].unsqueeze(1))
                xs[v] = xs[v].to(device)
            zs_outputs, zs_r, fusion_feature,views_center = model(xs, clustering=False)#[batch_size,d//2+1],[batch_size,d//2+1],[batch_size,view*(d//2+1)],
            ...
            if view_num==2:
                ...
                # loss_list.append(selfcontrastiveLoss.mcl_three_cluster_loss(views_center))
            elif view_num==4:
                loss_list.append(mcl_weight*selfcontrastiveLoss.mcl_four_loss(zs_outputs))
                loss_list.append(mcl_cluster_weight*selfcontrastiveLoss.mcl_four_cluster_loss(views_center))
            elif view_num==6:
                loss_list.append(mcl_weight*selfcontrastiveLoss.mcl_six_loss(zs_outputs))
                loss_list.append(mcl_cluster_weight*selfcontrastiveLoss.mcl_six_cluster_loss(views_center))
            for v in range(view_num):
                loss_list.append(rec_loss(xs[v]* mask[:, v].unsqueeze(1).to(device),zs_r[v]* mask[:, v].unsqueeze(1).to(device)))
        ...
        if (epoch + 1) % 1 == 0:
            with torch.no_grad():
                learned_features = []
                for x, _, idx, inpu ,mask in test_loader:
                    for v in range(view_num):
                        x[v] = x[v].to(device)
                    model.eval()
                    zs_outputs,zs_r,  fusion_feature,views_center= model(x)
...
if __name__ == '__main__':
    ...
    for i in range(5):
        # set_seed(seed)
        ACC, NMI, Purity, ARI, Fscore=main()
        acc_list.append(ACC)
        nmi_list.append(NMI)
        ari_list.append(ARI)
    ...
    log_message = 'ACC: ave|{:04f} std|{:04f}; '.format(np.mean(acc_list), np.std(acc_list, ddof=1))
    log_message += 'NMI: ave|{:04f} std|{:04f}; '.format(np.mean(nmi_list), np.std(nmi_list, ddof=1))
    log_message += 'PUR: ave|{:04f} std|{:04f}; '.format(np.mean(pur_list), np.std(pur_list, ddof=1))
    log_message += 'ARI: ave|{:04f} std|{:04f}; '.format(np.mean(ari_list), np.std(ari_list, ddof=1))
    log_message += 'Fscore: ave|{:04f} std|{:04f}'.format(np.mean(fscore_list), np.std(fscore_list, ddof=1))
